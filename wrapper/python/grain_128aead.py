...
"""
  Before using `grain_128aead` library module, make sure you've run
  ...
  Project: https://github.com/itzmeanjan/grain-128aead
"""
...
SO_PATH: str = abspath("../libgrain_128aead.so")
...
def encrypt(key: bytes, nonce: bytes, data: bytes, text: bytes) -> Tuple[bytes, bytes]:
    """
    Encrypts M ( >=0 ) -bytes plain text, with Grain-128 AEAD,
    ...
    """
    assert len(key) == 16, "Grain-128 AEAD takes 16 -bytes secret key !"
    assert len(nonce) == 12, "Grain-128 AEAD takes 12 -bytes nonce !"
    ...
    SO_LIB.grain_128aead_encrypt.argtypes = args

    SO_LIB.grain_128aead_encrypt(key_, nonce_, data_, ad_len, text_, enc, ct_len, tag)
...
def decrypt(
    ...
    """
    Decrypts M ( >=0 ) -bytes cipher text, with Grain-128 AEAD,
    while using 16 -bytes secret key, 12 -bytes public message nonce,
    ...
    assert len(key) == 16, "Grain-128 AEAD takes 16 -bytes secret key !"
    assert len(nonce) == 12, "Grain-128 AEAD takes 12 -bytes nonce !"
    assert len(tag) == 8, "Grain-128 AEAD takes 8 -bytes authentication tag !"
    ...
    SO_LIB.grain_128aead_decrypt.argtypes = args
    SO_LIB.grain_128aead_decrypt.restype = bool_t

    f = SO_LIB.grain_128aead_decrypt(
...
if __name__ == "__main__":
    print("Use `grain_128aead` as library module")
